...
(comment
  ...
  ; Win first, lose second, win overall because it's or
  (win-bet?
    {::method :or
     ::options
     [{::place #{0} ::racer #{"asdf"}}
  ...
  (win-bet?
    ...
    (s/explain :race/results [{:racer/id "asdf"}])
    (s/valid? :race/result {:racer/id "asdf"})
    (s/valid? :race/results [{:racer/id "asdf"}])
  )
